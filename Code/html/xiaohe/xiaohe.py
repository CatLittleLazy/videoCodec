...
def getTotalCompanyData():
	return requests.post(url = api[0], json = {"token":token}, headers={'Connection':'close'})
...
def getCompanyDetailDataFromNetOnce(data, companyType, date):
	...
	return requests.post(url = (api[1] + companyType), json = requestBody, headers={'Connection':'close'}).json()
